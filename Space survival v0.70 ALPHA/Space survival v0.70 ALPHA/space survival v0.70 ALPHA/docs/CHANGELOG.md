...
## v0.70 (Etap 6)
### Added
- Mechanika power-upów: `tarcza`, `bonus` punktowy oraz `życie` (dodatkowa szansa przetrwania).
- HUD tarczy z wyświetleniem statusu (BRAK / czas aktywności).
- Kolizje: aktywna tarcza pochłania pierwsze trafienie asteroidą.
- Leaderboard wyników (top 5) zapisywany lokalnie (`localStorage`).
- UI: modal leaderboard + dodatkowy wskaźnik skina w HUD oraz informacje „NAJLEPSZY / NOWY REKORD” na ekranie GAME OVER.

### Changed
- Pętla gry rozszerzona o spawn, ruch, kolizje i rysowanie power-upów.
